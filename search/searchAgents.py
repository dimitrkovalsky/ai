...
def cornersHeuristic(state, problem):
    ...
    x, y = state[0]
    costs = []
    for targetX, targetY in corners:
        costs.append(distance(x, y, targetX, targetY))
...
def foodHeuristic(state, problem):
    ...
    sum = 0
    for i in range(0, len(foodGrid.data)):
        for j in range(0, len(foodGrid.data[i])):
            if foodGrid.data[i][j]:
                sum += distance(position[0], position[1], i, j)
    return sum
